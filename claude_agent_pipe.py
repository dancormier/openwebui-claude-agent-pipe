...
class Pipe:
    class Valves(BaseModel):
        ANTHROPIC_API_KEY: str = Field(
            default="",
            description=(
                "Anthropic API key (pay-per-token), from the Claude Console. The "
                "path Anthropic's docs name for products built on the Agent SDK, "
                "and the only credential to use on an instance with more than "
                "one user. Leave empty to use CLAUDE_CODE_OAUTH_TOKEN or inherit "
        ...
        CLAUDE_CODE_OAUTH_TOKEN: str = Field(
            default="",
            description=(
                "Single-user installs only. Long-lived Claude Pro/Max/Team OAuth "
                "token from `claude setup-token`, authenticating your own "
                "subscription for your own use. Takes priority over "
                "ANTHROPIC_API_KEY (the key is unset so it can't override). "
                "Anthropic's docs say SDK-based products should use an API key "
                "and its position on subscription use has changed before; treat "
                "this path as unsupported and subject to change. Never set it on "
                "an instance other people use: routing their requests through "
                "your plan is what Anthropic's terms forbid."
            ),
        )
        MODEL: str = Field(
